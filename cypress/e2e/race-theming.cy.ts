...
describe('Race-Specific Theming', () => {
  const raceThemes = {
    ELF: {
      name: 'Elf',
      primaryColors: ['rgb(34, 139, 34)', 'rgb(0, 128, 0)', '#228b22', '#008000'],
      accentColors: ['rgb(255, 215, 0)', 'rgb(255, 193, 7)', '#ffd700', '#ffc107'],
      backgroundTones: ['rgb(240, 255, 240)', 'rgb(245, 255, 245)'],
      description: 'Nature-inspired green with gold accents',
    },
    GOBLIN: {
      name: 'Goblin',
      primaryColors: ['rgb(139, 69, 19)', 'rgb(160, 82, 45)', '#8b4513', '#a0522d'],
      accentColors: ['rgb(205, 92, 92)', 'rgb(178, 34, 34)', '#cd5c5c', '#b22222'],
      backgroundTones: ['rgb(245, 235, 220)', 'rgb(250, 240, 230)'],
      description: 'Earthy brown with red accents',
    },
    HUMAN: {
      name: 'Human',
      primaryColors: ['rgb(70, 130, 180)', 'rgb(100, 149, 237)', '#4682b4', '#6495ed'],
      accentColors: ['rgb(255, 215, 0)', 'rgb(192, 192, 192)', '#ffd700', '#c0c0c0'],
      backgroundTones: ['rgb(245, 247, 250)', 'rgb(248, 250, 252)'],
      description: 'Royal blue with silver accents',
    },
    UNDEAD: {
      name: 'Undead',
      primaryColors: ['rgb(64, 64, 64)', 'rgb(32, 32, 32)', '#404040', '#202020'],
      accentColors: ['rgb(128, 128, 128)', 'rgb(169, 169, 169)', '#808080', '#a9a9a9'],
  ...
  describe('ELF Theme', () => {
    it('should apply ELF green/gold color palette', () => {
      ...
      cy.get('[data-testid="race-header"]').should('have.css', 'background-color').and('match', /rgb\(34,\s*139,\s*34\)|rgb\(0,\s*128,\s*0\)/);
    ...
    it('should display ELF-specific shield icon', () => {
      ...
      cy.get('[data-testid="race-shield"] img').should('have.attr', 'src').and('include', 'ELF');
  ...
  describe('GOBLIN Theme', () => {
    it('should apply GOBLIN red/brown color palette', () => {
      ...
      cy.get('[data-testid="race-header"]').should('have.css', 'background-color').and('match', /rgb\(139,\s*69,\s*19\)|rgb\(160,\s*82,\s*45\)/);
      
      // Validate red accents
      cy.get('[data-testid="accent-element"]').should('have.css', 'color').and('match', /rgb\(205,\s*92,\s*92\)|rgb\(178,\s*34,\s*34\)/);
    ...
    it('should display GOBLIN-specific shield icon', () => {
      ...
      cy.get('[data-testid="race-shield"] img').should('have.attr', 'src').and('include', 'GOBLIN');
  ...
  describe('HUMAN Theme', () => {
    it('should apply HUMAN blue/gray color palette', () => {
      ...
      cy.get('[data-testid="race-header"]').should('have.css', 'background-color').and('match', /rgb\(70,\s*130,\s*180\)|rgb\(100,\s*149,\s*237\)/);
      
      // Validate silver/gray accents
      cy.get('[data-testid="accent-element"]').should('have.css', 'color').and('match', /rgb\(192,\s*192,\s*192\)|rgb\(169,\s*169,\s*169\)/);
    ...
    it('should display HUMAN-specific shield icon', () => {
      ...
      cy.get('[data-testid="race-shield"] img').should('have.attr', 'src').and('include', 'HUMAN');
  ...
  describe('UNDEAD Theme', () => {
    it('should apply UNDEAD gray/black color palette', () => {
      ...
      cy.get('[data-testid="race-header"]').should('have.css', 'background-color').and('match', /rgb\(64,\s*64,\s*64\)|rgb\(32,\s*32,\s*32\)/);
      
      // Validate silver accents
      cy.get('[data-testid="accent-element"]').should('have.css', 'color').and('match', /rgb\(128,\s*128,\s*128\)|rgb\(169,\s*169,\s*169\)/);
    ...
    it('should display UNDEAD-specific shield icon', () => {
      ...
      cy.get('[data-testid="race-shield"] img').should('have.attr', 'src').and('include', 'UNDEAD');
  ...
  describe('Theme Switching', () => {
    it('should switch between race themes correctly', () => {
      ...
      races.forEach((race) => {
        ...
        cy.get('[data-testid="race-header"]').should('have.css', 'background-color');
  ...
  describe('Gold Accent Text', () => {
    ...
    it('should apply gold gradient to important text', () => {
      ...
      cy.get('[data-testid="gold-gradient-text"]').should('have.css', 'background-clip').and('equal', 'text');
