...
Cypress.Commands.add('validateTouchTarget', (selector: string, minSize: number = 48) => {
  cy.get(selector).then(($el) => {
    ...
    expect(width).to.be.at.least(minSize, `Touch target width ${width}px is below minimum ${minSize}px`);
    expect(height).to.be.at.least(minSize, `Touch target height ${height}px is below minimum ${minSize}px`);
  });
});

// Color contrast validation (WCAG AA: minimum 4.5:1 for normal text)
Cypress.Commands.add('validateContrast', (selector: string, minRatio: number = 4.5) => {
  cy.get(selector).then(($el) => {
    const fgColor = $el.css('color');
    const bgColor = $el.parent().css('background-color');
    // Use Vision MCP to validate contrast ratio
    cy.captureScreenshot(`contrast-${selector.replace(/[^a-zA-Z0-9]/g, '-')}`);
...
Cypress.Commands.add('switchRace', (race: 'ELF' | 'GOBLIN' | 'HUMAN' | 'UNDEAD') => {
  cy.visitApp('/test');
...
Cypress.Commands.add('validateAria', (selector: string, attributes: Record<string, string>) => {
  cy.get(selector).then(($el) => {
...
Cypress.Commands.add('navigateWithKeyboard', (selectors: string[], direction: 'tab' | 'shift-tab' = 'tab') => {
...
Cypress.Commands.add('waitForAnimation', (selector: string, duration: number = 500) => {
  cy.get(selector).should('be.visible');
...
Cypress.Commands.add('validateGradient', (selector: string) => {
  cy.get(selector).should('have.css', 'background-image').and('match', /gradient/);
  cy.captureScreenshot(`gradient-${selector.replace(/[^a-zA-Z0-9]/g, '-')}`);
});

// Validate shadow depth
Cypress.Commands.add('validateShadow', (selector: string, expectedShadow: string) => {
...
Cypress.Commands.add('validateGoldText', (selector: string) => {
  cy.get(selector).should('have.css', 'color').and('match', /rgb\(255,\s*215,\s*0\)|rgb\(255,\s*193,\s*7\)|#ffd700|#ffc107/);
...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      validateAria(selector: string, attributes: Record<string, string>): Chainable<void>;
      navigateWithKeyboard(selectors: string[], direction?: 'tab' | 'shift-tab'): Chainable<void>;
