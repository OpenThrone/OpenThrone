...
const handler = async(
  ...
  if (req.method === 'GET') {
    ...
    if (history >= 5) {
      return res
        .status(400)
        .json({ error: 'You can only Recruit up to 5x in 24 hours. A', IP: req.headers['cf-connecting-ip'] });
    }
  ...
  if (req.method === 'POST') {
    ...
    if (history >= 5) {
      return res
        .status(400)
        .json({ error: 'You can only Recruit up to 5x in 24 hours. B' + req.headers['cf-connecting-ip'] });
    }
