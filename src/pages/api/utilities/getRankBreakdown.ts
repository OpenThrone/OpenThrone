...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const calculateUserScoreNEW = (user) => {
    const sortedItems = JSON.parse(
      JSON.stringify(user.items.sort((a, b) => b.level - a.level)),
    );
    const sortedUnits = JSON.parse(
      JSON.stringify(user.units.sort((a, b) => b.level - a.level)),
    );
    ...
    const itemScore = user.items
      ? user.items
          .map(
            (item) =>
              (ItemTypes.find(
                (i) => i.level === item.level && i.type === item.type,
              )?.bonus || 0) * item.quantity,
          )
          .reduce((a, b) => a + b, 0)
      : 0;
