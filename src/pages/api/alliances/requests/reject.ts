...
const rejectRequestHandler = async (req: any, res: any) => {
  if (req.method !== 'POST') {
  ...
  try {
    const result = await AllianceService.rejectJoinRequest(requestId, user.id);
...
export default withAuth(rejectRequestHandler);
