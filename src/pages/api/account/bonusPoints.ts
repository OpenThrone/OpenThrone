...
const handler = async (
  ...
  try {
    const result = await AccountService.updateBonusPoints(userId, { changeQueue: changeQueue as Record<string, { change: number }> });
    ...
    if (error.message?.startsWith('Not enough proficiency points') || error.message?.startsWith('Cannot reduce level') || error.message?.startsWith('Cannot increase level')) {
