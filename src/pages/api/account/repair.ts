...
const handler = async (
  ...
  if (!req.session?.user?.id) {
    logError(
      null,
      { requestPath: req.url },
      'Auth session missing in repair handler',
    );
  ...
  try {
    const result = await AccountService.repairFortification(userId, {
      repairPoints,
    });
    ...
    const logContext = parseResult.success
      ? { userId, repairPoints: parseResult.data.repairPoints }
    ...
    if (
      ...
      error.message?.startsWith('Fortification is already at full health') ||
      error.message?.startsWith('Invalid fortification level') ||
      error.message?.startsWith('Calculated repair amount')
    ) {
    ...
    return res.status(500).json({
      error: 'An unexpected error occurred while repairing fortification.',
    });
  }
};
