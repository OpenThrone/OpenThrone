...
describe('API admin/users', () => {
  ...
  it('GET returns formatted user response', async () => {
    const req: any = { method: 'GET', session: { user: { id: 1 } }, query: { userId: '1' } };
    const mockUser = { id: 1, display_name: 'bob', email: 'b@b.com', gold: 100, gold_in_bank: 10, UserUnit: [{ id: 0, userId: 1, type: 'CITIZEN', level: 1, quantity: 10, isMercenary: false }], UserItem: [{ id: 0, userId: 1, type: 'SWORD', level: 1, quantity: 1, usage: 'GENERAL' }], permissions: [], units: [{ type: 'CITIZEN', level: 1, quantity: 10 }], items: [{ type: 'SWORD', level: 1, quantity: 1, usage: 'GENERAL' }] };
