...
import { PermissionType, ReportPriority, ReportResolution, ReportStatus } from '@prisma/client';
...
const ReportDetailsPage = () => {
    ...
    try {
      const response = await fetch(`/api/reports/${reportId}/actions`, {
        ...
        body: JSON.stringify({
          ...
          toPriority: actionType === 'PRIORITY_CHANGED' ? newPriority : undefined,
        }),
      });
  ...
  return (
    <PermissionCheck permission={PermissionType.REVIEW_REPORTS}>
      <MainArea title={`Report Details #${report.id}`}>
        <Grid>
            ...
            <Stack gap="md">
              ...
              {report.chatMessage && (
                <GameCard title="Linked Chat Message">
                  <Text size="sm" c="dimmed">
                    Sent at: {new Date(report.chatMessage.sentAt).toLocaleString()}
                  </Text>
              ...
              <GameCard title="Actions Timeline">
                <Timeline active={(report.actions || []).length - 1} bulletSize={24} lineWidth={2}>
          ...
          <Grid.Col span={{ base: 12, md: 4 }}>
            <Stack gap="md">
              {!report.assignedToUserId && (
                <Button color="blue" fullWidth onClick={handleAssign} loading={assigning}>
                  Claim / Self-Assign
                </Button>
              )}
              ...
              {report.status !== 'RESOLVED' && (
                <GameCard title="Resolve Report">
                  <Stack gap="sm">
                    ...
                    />
                    <Button color="green" fullWidth onClick={handleResolve} loading={resolving}>
                      Resolve Report
                    </Button>
                  </Stack>
                </GameCard>
              ...
              {report.status !== 'RESOLVED' && (
                <GameCard title="Manage Status & Priority">
                  <Stack gap="sm">
                    ...
                    <Button color="orange" fullWidth onClick={handleAddAction} loading={addingAction}>
                      Update Action
                    </Button>
                  </Stack>
                </GameCard>
