...
import { PermissionType, SignalSeverity } from '@prisma/client';
...
const CheatSignalsPage = () => {
  ...
  const handleAction = async (id: number, action: 'acknowledge' | 'dismiss' | 'takeAction') => {
    setActionId(id);
    try {
  ...
  return (
    <PermissionCheck permissions={['VIEW_CHEAT_SIGNALS']}>
      <MainArea title="Abuse: Cheat Signal Dashboard">
        <Stack gap="md">
              ...
              <Table>
                ...
                <Table.Tbody>
                  {signals.map((s) => (
                    <Table.Tr key={s.id}>
                      <Table.Td>
                        <Badge color={severityColor(s.severity)}>{s.severity}</Badge>
                      </Table.Td>
