...
import { Badge, Button, Center, Group, NumberInput, RingProgress, SimpleGrid, Space, Stack, Table, Text, Tooltip } from '@mantine/core';
import { BiCoinStack, BiWrench, BiHistory } from 'react-icons/bi';
...
const Repair = () => {
  ...
  const healthPercentage = Math.floor((user.fortHitpoints / fortification.hitpoints) * 100);
  
  const statItems = [
    { label: "Gold In Hand", value: toLocale(user.gold, user.locale), icon: <BiCoinStack size={18} /> },
  ...
  const historyRows = history.map(entry => (
    <Table.Tr key={entry.id}>
      ...
      <Table.Td>{entry.stats.currentFortHP} <FontAwesomeIcon icon={faArrowRight} /> {entry.stats.newFortHP}</Table.Td>
  ...
  return (
    <MainArea title="Repair Fortification">
      ...
      <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md">
        <GameCard title="Fortification Status" icon={faArrowRight} goldAccent={remainingHitpoints > 0}>
          <Group justify="space-between">
            <Stack gap="xs">
              <Text size="xl" fw={700}>{fortification.name}</Text>
              <Badge color={remainingHitpoints > 0 ? 'red' : 'green'}>{remainingHitpoints > 0 ? 'Damaged' : 'Fully Repaired'}</Badge>
            </Stack>
            <RingProgress sections={[{ value: healthPercentage, color: healthPercentage < 50 ? 'red' : 'yellow' }]} label={<Center><Text>{healthPercentage}%</Text></Center>} />
          </Group>
          <Text mt="md">HP: {user.fortHitpoints.toLocaleString()} / {fortification.hitpoints.toLocaleString()}</Text>
        </GameCard>
        <GameCard title="Repair" icon={<BiWrench size={16} />}>
          <NumberInput
            label="Amount to Repair"
            value={repairPoints}
            onChange={(value) => setRepairPoints(typeof value === 'number' ? value : 0)}
            min={0}
            max={remainingHitpoints}
          />
          <Text size="sm" c="dimmed" mt="xs">Cost: {toLocale(repairPoints * fortification.costPerRepairPoint, user.locale)} gold</Text>
          <Group mt="md">
            <Button onClick={() => handleRepair(repairPoints)} disabled={repairPoints <= 0 || isRepairing}>Repair</Button>
            <Button onClick={() => handleRepair(remainingHitpoints)} disabled={remainingHitpoints <= 0 || isRepairing}>Repair All</Button>
          </Group>
        </GameCard>
      </SimpleGrid>
      <Space h="md" />
