...
const UpgradeTab = () => {
  ...
  const tabParam = Array.isArray(router.query.tab)
    ? router.query.tab[0]
    : router.query.tab;
  const tab = tabParam || 'fortifications';
  ...
  const getTabTitle = (currentTab: string) => {
    switch (currentTab) {
      ...
      case 'offense':
        return 'Siege Upgrades';
      case 'intel':
        return 'Clandestine Upgrades';
      case 'armory':
        return 'Armory Upgrades';
      case 'houses':
        return 'Housing Upgrades';
      case 'economy':
        return 'Economy Upgrades';
      default:
    ...
    <MainArea title={getTabTitle(tab)}>
      <Box className="rpg-inset" p="xs" style={{ borderRadius: '6px' }}>
        <Tabs
          ...
          onChange={(value) => {
            if (value) {
              router.push(`/structures/upgrades/${value}`);
            }
          }}
          variant="pills"
          color="yellow"
