...
describe('AttackService', () => {
  ...
  describe('calculateStrength', () => {
    it('returns non-zero for populated offense', () => {
      ...
      expect(
        strength.totalStats.MeleeAtkPower + strength.totalStats.RangedAtkPower,
      ).toBeGreaterThan(0);
    });
    ...
    it('returns non-zero for populated defense', () => {
      ...
      expect(
        strength.totalStats.MeleeDefPower + strength.totalStats.RangedDefPower,
      ).toBeGreaterThan(0);
    });
  });
  ...
  describe('simulateBattle', () => {
    it('results in attacker win with low defender units', async () => {
      ...
      userGenerator.addUnits(
        normUnits([
          {
            id: 0,
            userId: userGenerator.getPrismaUser().id,
      ...
      const defenderForTest = new UserModel(
        ...
        userGenerator.getBattleUpgrades().map((b) => ({
          id: b.id,
          userId: b.userId,
          type: b.type as BattleUpgradeType,
          level: b.level,
      ...
      const result = await AttackService.simulateBattle(
        attacker,
        defenderForTest,
        100,
        1,
      ...
      expect(result.Losses.Attacker.total).toBeLessThanOrEqual(
        result.Losses.Defender.total,
      );
    ...
    it('results in defender win with high defender units', async () => {
      ...
      expect(result.Losses.Attacker.total).toBeGreaterThanOrEqual(
        result.Losses.Defender.total,
      );
  ...
  describe('executeAttack', () => {
    it('persists mitigation metadata into attack_log.stats', async () => {
      ...
      defenderGen.setFortHitpoints(
        Fortifications.find((f) => f.level === 24)?.hitpoints ?? 9500,
      );
      ...
      expect(typeof createdAttackLogPayload.stats.mitigation_log).toBe(
        'string',
      );

      const mitigationLog = JSON.parse(
        createdAttackLogPayload.stats.mitigation_log,
      );
