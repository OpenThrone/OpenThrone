...
import { playerUnit, structureUpgrade, userFixture } from 'test/utils/factories';
import type { UnitType, PlayerUnit, StructureUpgrade } from '@/types/typings';
...
describe('UserStatsService / UserUnitsService parity checks (spy/sentry)', () => {
  test('attacker with many spies should have higher spy stat than defender sentry (attacker advantage)', () => {
    const attackerData = userFixture({
      units: [playerUnit('SPY', 1, 2000), playerUnit('SPY', 2, 1000), playerUnit('SPY', 3, 1000)],
  ...
  const attackerSpy = attackerStats.calculateArmyStat('SPY').totalStats.MeleeAtkPower;
  const defenderSentry = defenderStats.calculateArmyStat('SENTRY').totalStats.MeleeDefPower;
  ...
  const defenseStatObj = defenderStats.calculateArmyStat('DEFENSE').totalStats;
  const sentryStatObj = defenderStats.calculateArmyStat('SENTRY').totalStats;

  const defenseStat = (defenseStatObj.MeleeDefPower || 0) + (defenseStatObj.RangedDefPower || 0);
