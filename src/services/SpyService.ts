...
export const SpyService = {
  ...
  async executeSpyMission(attackerId: number, defenderId: number, spies: number, type: 'INTEL' | 'ASSASSINATE' | 'INFILTRATE', unit?: UnitType | typeof CITIZEN_WORKERS_TARGET) {
    const validatedData = SpyMissionSchema.parse({ attackerId, defenderId, spies, type, unit });
    ...
    try {
      const prismaTx = await prisma.$transaction(async (tx) => {
        if (validatedData.type === 'INTEL') {
          spyResults = this.simulateIntel(attacker, defender, validatedData.spies);
        } else if (validatedData.type === 'ASSASSINATE') {
          spyResults = this.simulateAssassination(attacker, defender, validatedData.spies, validatedData.unit);

          await updateUserUnits(validatedData.defenderId,
            defender.units as any as import('@/types/typings').PlayerUnit[],
            tx
          );
        } else if (validatedData.type === 'INFILTRATE') {
          spyResults = this.simulateInfiltration(attacker, defender, validatedData.spies);
          await updateUserUnits(validatedData.defenderId,
            defender.units as any as import('@/types/typings').PlayerUnit[],
            tx
          );
        ...
        if (spyResults.spiesLost > 0) {
          ...
          await updateUserUnits(validatedData.attackerId,
        ...
        await incrementUserStats(validatedData.attackerId, {
          type: 'SPY',
          subtype: (validatedData.attackerId === Winner.id) ? 'WON' : 'LOST',
        }, tx);
        logDebug('Incremented attacker stats', { attackerId, subtype: (validatedData.attackerId === Winner.id) ? 'WON' : 'LOST' });
        
        await incrementUserStats(validatedData.defenderId, {
          type: 'SENTRY',
          subtype: (validatedData.defenderId === Winner.id) ? 'WON' : 'LOST',
        }, tx);
        logDebug('Incremented defender stats', { defenderId, subtype: (validatedData.defenderId === Winner.id) ? 'WON' : 'LOST' });
