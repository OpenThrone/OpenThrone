  ...
  describe('Spy Mission Simulations', () => {
    ...
    describe('simulateIntel', () => {
      it('should succeed with high intel accuracy/quantity when attacker spy >> defender sentry', () => {
        ...
        baseAttacker.units.find(
          (u) => u.type === 'SPY' && u.level === 1,
        )!.quantity = 2000; // More spies
        logDebug(
          'SentryUnits',
          baseDefender.units.find((u) => u.type === 'SENTRY' && u.level === 1)
            .level,
        );
        logDebug('Sentry Stats', baseDefender.sentry);
        baseDefender.units.find(
          (u) => u.type === 'SENTRY' && u.level === 1,
        )!.quantity = 100;
        logDebug(
          'SentryUnits',
          baseDefender.units.find((u) => u.type === 'SENTRY' && u.level === 1)
      ...
      it('should fail with high spy losses when attacker spy << defender sentry', () => {
        ...
        const spiesSent = 10;
        const initialSpyCount = baseAttacker.units.find(
          (u) => u.type === 'SPY' && u.level === 1,
        )!.quantity;
        const result = simulateIntel(baseAttacker, baseDefender, spiesSent);
        ...
        const finalSpyCount = baseAttacker.units.find(
          (u) => u.type === 'SPY' && u.level === 1,
        )!.quantity;
      ...
      it('should handle defender having zero sentry stat', () => {
        baseDefender.units = baseDefender.units.filter(
          (u) => u.type !== 'SENTRY',
        ); // Remove sentries
    ...
    describe('simulateAssassination', () => {
      it('should succeed and kill target units (CITIZEN/WORKERS) with low spy losses when attacker >> defender', () => {
        ...
        const initialAssassinCount = baseAttacker.units.find(
          (u) => u.type === 'SPY' && u.level === 3,
        )!.quantity;
        const initialCitizenCount = baseDefender.units.find(
          (u) => u.type === 'CITIZEN',
        )!.quantity;
        const initialWorkerCount = baseDefender.units.find(
          (u) => u.type === 'WORKER',
        )!.quantity;

        const result = simulateAssassination(
          baseAttacker,
          baseDefender,
          spiesSent,
          CITIZEN_WORKERS_TARGET,
        ...
        const finalAssassinCount = baseAttacker.units.find(
          (u) => u.type === 'SPY' && u.level === 3,
        )!.quantity;
        const finalCitizenCount = baseDefender.units.find(
          (u) => u.type === 'CITIZEN',
        )!.quantity;
        const finalWorkerCount = baseDefender.units.find(
          (u) => u.type === 'WORKER',
        )!.quantity;

        expect(finalAssassinCount).toBe(
          initialAssassinCount - result.spiesLost,
        );
        expect(finalCitizenCount + finalWorkerCount).toBe(
          initialCitizenCount + initialWorkerCount - result.unitsKilled,
        );
      });
      ...
      it('should succeed but with spy losses when attacker > defender (closer stats)', () => {
        ...
        const initialTargetCount = baseDefender.units.find(
          (u) => u.type === 'DEFENSE',
        )!.quantity;
        ...
        const finalTargetCount = baseDefender.units.find(
          (u) => u.type === 'DEFENSE',
        )!.quantity;
        expect(finalAssassinCount).toBe(
          initialAssassinCount - result.spiesLost,
    ...
    describe('simulateInfiltration', () => {
      it('should succeed and cause fort damage with low spy losses when attacker >> defender', () => {
        ...
        const initialInfiltratorCount = baseAttacker.units.find(
          (u) => u.type === 'SPY' && u.level === 2,
        )!.quantity;
        const initialFortHP = baseDefender.fortHitpoints;

        const result = simulateInfiltration(
          baseAttacker,
          baseDefender,
          spiesSent,
        );
        ...
        const finalInfiltratorCount = baseAttacker.units.find(
          (u) => u.type === 'SPY' && u.level === 2,
        )!.quantity;
        expect(finalInfiltratorCount).toBe(
          initialInfiltratorCount - result.spiesLost,
        );
