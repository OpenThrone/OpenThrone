...
const NewItemSection: React.FC<any> = React.memo(({ heading, items, itemCosts, setItemCosts, units }) => {
  ...
  const handleEquip = async (operation: 'buy' | 'sell') => {
    const itemsToProcess = currentItems
      ...
      .map(item => ({
        ...item,
        quantity: operation === 'sell' ? Math.min(itemCosts[item.id], item.ownedItems || 0) : itemCosts[item.id],
      }))
    ...
    try {
      const response = await fetch(`/api/armory/${operation === 'buy' ? 'equip' : 'unequip'}`, {
        method: 'POST',
    ...
    <GameCard title={heading}>
      <StyledTable headers={['Item', 'Details', 'Owned', 'Quantity']}>
        {currentItems.map(unit => (
          <Table.Tr key={unit.id}>
            ...
            <Table.Td>+{unit.bonus} {unit.usage}</Table.Td>
      ...
      <Group justify="space-between" mt="md">
        ...
        <Group>
          ...
          <Button onClick={() => handleEquip('sell')} color="red">Sell</Button>
        </Group>
      </Group>
    </GameCard>
  );
