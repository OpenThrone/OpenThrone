...
import { NumberInput, Table, Title, Tooltip } from '@mantine/core';
...
const ItemsInputForm: React.FC<ItemsInputFormProps> = ({ items, onChange }) => {
  ...
  const itemTypes = useMemo(
    () => ['WEAPON', 'HELM', 'ARMOR', 'BOOTS', 'BRACERS', 'SHIELD'],
    [],
  );
  ...
  const itemsWithQuantities = useMemo(() => {
    return itemTypes.map((type) => {
      ...
      const itemTypeData = ItemTypes.find((item) => item.type === type) || {
        name: type,
        usage: 'GENERAL',
      };
  ...
  const handleItemChange = useCallback(
    (type: string, level: number, quantity: number, _usage: string) => {
