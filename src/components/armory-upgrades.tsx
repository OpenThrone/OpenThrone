      ...
      <Table.Tr key={`${item.name}_${item.level}`}>
        ...
        <Table.Td>
          {item.level > userLevel && item.fortLevel <= fortLevel ? (
            <Button onClick={() => buyUpgrade('armory', index, forceUpdate)}>Buy</Button>
          ) : (
            item.level > userLevel && <span>Unlock with Fort: {Fortifications.find(f => f.level === item.fortLevel)?.name}</span>
          )}
        </Table.Td>
      </Table.Tr>
    ));
