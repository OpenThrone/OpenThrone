...
const renderOutcome = (log: Log, type: string) => {
  if (type === 'defense') {
    return log.winner === log.defender_id ? (
      <FontAwesomeIcon icon={faCheck} color="lightgreen" size="lg" />
    ) : (
      <FontAwesomeIcon icon={faXmark} color="red" size="lg" />
    );
  }
  return log.winner === log.attacker_id ? (
    <FontAwesomeIcon icon={faCheck} color="lightgreen" size="lg" />
  ) : (
    <FontAwesomeIcon icon={faXmark} color="red" size="lg" />
  );
...
const PlayerOutcome: React.FC<PlayerOutcomeProps> = ({
  log,
  type,
  collapsed = true,
}) => {
