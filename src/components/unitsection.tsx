...
import type { UnitProps, UnitType } from '@/types/typings'; // Import UnitType
...
const UnitSection: React.FC<UnitSectionProps> = ({
  ...
  const handleTrain = async () => {
    const unitsToTrain = units
      ...
      .filter(
        (unit): unit is { type: UnitType; quantity: number; level: number } =>
          unit !== null,
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.data && Array.isArray(data.data)) {
          ...
          logWarn(
            'Train API response did not contain expected data structure:',
            data,
          );
    ...
    const unitsToUntrain = units
      ...
      .map((unit) => {
        ...
        if (quantity > (unit.ownedUnits ?? 0)) {
          alertService.error(
            `Cannot untrain ${quantity} ${unit.name}(s), you only own ${unit.ownedUnits ?? 0}.`,
          );
      ...
      .filter(
        (unit): unit is { type: UnitType; quantity: number; level: number } =>
          unit !== null && unit !== 'INVALID_QUANTITY',
      ); // Filter out nulls and invalid quantities
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.data && Array.isArray(data.data)) {
          ...
          logWarn(
            'Untrain API response did not contain expected data structure:',
            data,
          );
      ...
      logError('Untraining error:', error);
        ...
        <tbody>
          {(() => {
            ...
            return sortedUnits.map((unit) => {
              ...
              if (unit.id === firstDisabledUnit?.id) {
