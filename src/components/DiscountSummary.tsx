...
const DiscountSummary: React.FC<Props> = ({
  user,
  baseCost,
  discountedCost,
}) => {
  ...
  const breakdown = useMemo(() => {
    ...
    const player = (user.playerBonuses || [])
      .filter((b: any) => b.bonusType === 'PRICES')
      .map((b: any) => ({
        label: `${String(b.race || b.bonusType)}`,
        percent: b.bonusAmount ?? 0,
      }));
    const points = (user.bonus_points || [])
      .filter((p: any) => p.type === 'PRICES')
      .map((p: any) => ({
        label: `Proficiency Points`,
        percent: p.level ?? 0,
      }));
      ...
      label={
        <div style={{ minWidth: 220 }}>
          <Text size="sm" fw={600} mb={6}>
            Discount breakdown — total {Number(totalPct).toFixed(2)}%
          </Text>
          {breakdown.length > 0 ? (
            breakdown.map((d, i) => (
              <div
              ...
              >
                ...
                <Text size="sm" fw={600}>
                  {Number(d.percent).toFixed(2)}%
                </Text>
              </div>
            ))
          ) : (
            <Text size="sm" c="dimmed">
              No individual sources (total shown)
            </Text>
          )}
          <hr
            style={{
