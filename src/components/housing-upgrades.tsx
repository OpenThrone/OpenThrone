...
const HousingTab: React.FC<BattleUpgradeProps> = ({ userLevel, fortLevel, forceUpdate }) => {
      ...
      <Table.Tr key={item.index}>
        ...
        <Table.Td>
          {item.index === userLevel + 1 && item.fortLevel <= fortLevel ? (
            <Button onClick={() => buyUpgrade('houses', index, forceUpdate)}>Buy</Button>
          ) : (
            item.index > userLevel && <span>Unlock with {Fortifications.find(f => f.level === item.fortLevel)?.name}</span>
          )}
        </Table.Td>
      </Table.Tr>
    ));
  ...
  return (
    <StyledTable headers={['Name', 'Fort Req.', 'Citizens Per Day', 'Cost', 'Action']}>
      {rows}
    </StyledTable>
  );
};
