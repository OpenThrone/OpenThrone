...
const OffenseUpgrade: React.FC<BattleUpgradeProps> = ({ userLevel, fortLevel, forceUpdate }) => {
  const { user } = useUser();
      ...
      <Table.Tr key={`${item.name}_${item.level}`}>
        ...
        <Table.Td>
          {item.level === userLevel + 1 && item.fortLevelRequirement <= fortLevel ? (
            <Button onClick={() => buyUpgrade('offense', index, forceUpdate)}>Buy</Button>
          ) : (
            item.fortLevelRequirement > fortLevel && <span>Unlock with: {Fortifications.find(f => f.level === item.fortLevelRequirement)?.name}</span>
          )}
        </Table.Td>
      </Table.Tr>
    ));
